...
async function runAudit(){
  ...
  try{
    const auditAnswers=Object.entries(answerLabels).map(([q,label])=>{const score=v[parseInt(q)];const level=score===0?'Critical gap':score===1?'Needs work':score===2?'Getting there':'Strong';return {area:bl[parseInt(q)],label:label,score:score,level:level};});
